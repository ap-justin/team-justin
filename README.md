...
- Read them back: `/team-justin:todos [n | substring]` and `/team-justin:issues [slug | substring]` — the singular writes one, the plural prints them. Bare, each lists what the store holds — wants in capture order; defects severity-first, stubs tagged as stubs, closing on a **concentration** count (severity split, and the module the most write-ups cite). With an argument, each prints that one entry **verbatim**. Read-only: no investigation, no triage, no ranking, no edits — an empty `issues/` really does mean no known defects, since a fix deletes the file.
