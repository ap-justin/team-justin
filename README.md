...
- Mint the repo's own lead: `/team-justin:deploy`. Run it once per repo. It derives that repo's standing answers — which seat owns which stack, and whatever the repo's own docs leave unwritten (the gate that closes its token file, **what a test run costs**) — and writes them into its `.claude/CLAUDE.md` as one marked block of about 400 tokens: a pointer at `team-justin:lead` plus that sheet. The pointer is the design. *How the team works* is needed only once work starts, so it stays in the plugin and loads on demand; *who is on the team here* is what tells a session there is a team at all, so it is the only part that goes ambient. Every line cites what it came from, and the block is stamped with the plugin version it came from — so re-running is both the "the repo changed" fix and the "the plugin changed" one.
