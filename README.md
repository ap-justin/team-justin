...
## Layout
- `skills/lead/SKILL.md`: the lead / orchestrator; invokes as `/team-justin:lead` (add the `brief` verb to grill a change and persist its brief). **You are the PM**: the team formalizes and executes the subject you bring.
...
- Plan it first: `/team-justin:lead brief <subject>`. The lead grills you to the studs, then writes a change-shaped brief to the plan store: what lands, blast radius, **cadence**, decisions, non-goals, done-when. Cadence is the one to get right: commits are the steps, and a PR split means an environment boundary or a partial ship. A bare `lead <task>` still grills when the work warrants it; only `brief` persists. (`lead plan` is the same verb.)
