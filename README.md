...
- Read them back: `/team-justin:todos [n | substring]` and `/team-justin:issues [slug | substring]` — the singular writes one, the plural prints them. Bare, each lists what the store holds — wants ranked `value ÷ effort` (high value, low effort first), numbered by capture position so a number stays a stable handle, unscored `?` lines after rather than guessed into the order, and anything 90+ days old or filed against an archived effort marked stale; defects severity-first, stubs tagged as stubs, closing on a **concentration** count (severity split, and the module the most write-ups cite). With an argument, each prints that one entry **verbatim**. Read-only: no investigation, no triage, no scoring, no edits — both order what's filed and neither picks off it, and an empty `issues/` really does mean no known defects, since a fix deletes the file.
