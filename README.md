...
- **Conformance is prevented, not detected** — whether a build honors its design is a judgment a person makes in a glance and an agent can't make at all, so nothing tries to. The token file is a **closed set**: builders write no value that isn't in it and return a **named gap** instead of inventing one; the handoff names what's available; `taste-reviewer` greps statically for values outside the system; `visual-reviewer` supplies measurements, plus an **outside eye** — a Gemini vision model reading the same screenshots, which describes and flags visible breakage and is prompted to leave the verdict, the score and every measurement to its reader. What it catches routes to the lead and the builders, gated on **blast radius, not provenance**: a cheap reversible fix is taken, a change to the system or a settled direction comes back as a named proposal; `accessibility-reviewer` supplies WCAG criteria. All three are lead-dispatchable seats **and** user-invocable slash passes off one shared body — but none of them returns a verdict on intent. The verdict stays the user's.
