...
# rule, or no learnings channel — or a planner brief naming no brief.md — and
...
if [ -n "$canon" ]; then
  ...
  [ -n "$hit" ] && reasons="${reasons}restates an always-loaded rule verbatim: \"${hit#*	}\" is in ${hit%%	*} — point at the file instead (scan 2). "
fi
