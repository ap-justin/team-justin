...
**The line leaves when the work lands.** `IDEAS.md` is the **open** parking lot, not an archive — the same rule that makes an empty `issues/` a true claim. A slice that satisfies a parked want **deletes its line** in the same Step 4.5 reconciliation that commits the work, because the commit is where the want is written down now. The verb that does this in bulk is **`/team-justin:lead pull`** (`skills/lead/references/pull-verb.md`): it proposes a batch off the ranked file, the user picks, and each line it lands is retired. It's also the only place a filed `value`/`effort` is corrected — a want that turns out bigger than its number is **rescored in place and left parked**, never half-built to justify the pull. Everything the batch never reached stays exactly as filed.
