...
`react-hook-form` in `package.json` means load the **`react-hook-form`** skill before wiring a form to a server function. `handleSubmit` calls `preventDefault()` unconditionally, so pairing it with a `<form action={...}>` validates and then runs nothing. Wire one boundary: keep `onSubmit={handleSubmit(...)}` and call the `createServerFn` RPC inside the valid handler with RHF's data, then map the result through `setError` — a field path for anything the user can fix by editing, `root.server` for the rest. The server function validates the same schema on its own side. Note the two shapes: with a transforming or coercing schema, the handler's argument is the schema's **output** while `getValues`/`watch` stay on the input — type it `useForm<Input, unknown, Output>` or the RPC's parameter lies. `reference/wiring.md` has the submit ladder.
