...
Your first slice isn't a component. It's the design system, as two real artifacts: **the token file** in the app's own style layer, never inside `design/`; and **the conformance gate** in the repo's own test runner, in the same slice — every value a component writes resolves to that file, and a literal is a finding, returned rather than asserted so the caller's `expect(...).toEqual([])` prints the file and the value. **You transcribe the design's values; you don't tune them.** A value you'd have picked differently is not a defect, and that includes colour: **contrast is the design's and ships as authored — no seat on this team computes a ratio.** A value the design doesn't have is a named gap and goes back to the user.
