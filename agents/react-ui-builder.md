...
- **The conformance gate**, in the repo's own test runner, in the same slice. Two sweeps: **values** — every colour, length, type bundle, radius, duration and easing a screen writes resolves to the token file, and a literal is a finding; **names** — every class a component writes is one the stylesheets actually draw, because a name nothing draws paints nothing and still looks correct in review. Return findings rather than asserting them, so the caller's `expect(...).toEqual([])` prints the file and the name. What a gate globs is per-surface; what counts as a violation is not.
