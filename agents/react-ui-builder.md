...
- **The token file**, at the destination the plan names — this stack's own style layer (`src/app.css`, `app/globals.css`, `packages/ui/tokens.css` in a monorepo), **never inside `design/`**. Every step of every scale as a named custom property: the color ramps, spacing, type, radius, elevation, durations, easings. This artifact is permanent — components consume it and never supersede it — so siting it where the app actually ships means it never has to migrate later. Add the `## Design system` pointer to `CLAUDE.md` if `design-director` hasn't.
