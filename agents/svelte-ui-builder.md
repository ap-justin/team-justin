...
- Banned inside components: the route `data` prop shape (`PageData`), `$app/server`, `$env/*/private`, DB clients, fetch-in-effect for server data. If a component needs a form, render the fields and take an `action`/callback prop — the SvelteKit seat owns the action + progressive enhancement.
