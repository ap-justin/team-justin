...
- **The conformance gate**, in the repo's own test runner, in the same slice: every colour, length, type bundle, radius, duration and easing a screen writes resolves to the token file, and a literal is a finding. Return findings rather than asserting them, so the caller's `expect(...).toEqual([])` prints the file and the value.
