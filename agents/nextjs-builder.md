...
`react-hook-form` in `package.json` means load the **`react-hook-form`** skill before wiring a form to the server. `handleSubmit` calls `preventDefault()` unconditionally, so `<form action={serverAction} onSubmit={handleSubmit(...)}>` validates, reports success, and **never runs the action** — while `action` alone skips RHF entirely and hands the action a `FormData` missing every `Controller` field. Wire one boundary: keep `onSubmit={handleSubmit(...)}` and call the `'use server'` action inside the valid handler with RHF's data (a server action takes any serializable argument, so it needs no `FormData`), then map what comes back through `setError` — a field path for anything the user can fix by editing, `root.server` for the rest. The action re-validates with the same schema; the client's parse is the affordance, yours is the boundary. `reference/wiring.md` has the submit ladder and the progressive-enhancement variant.
