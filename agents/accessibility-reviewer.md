...
- **No dev server** → audit the source statically (semantics, labels, `alt`, ARIA, focus management, `tabindex`, reachable handlers, literal token pairs whose contrast you can actually compute) and mark every rendered criterion **not verified** rather than passed.
