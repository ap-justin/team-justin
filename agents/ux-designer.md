...
- **On greenfield your flow is what the design system gets built against.** The lead's greenfield order is: your flow → `design-director` writes a **binding plan + a coverage manifest** from it → a UI builder renders that system as a real token file plus a static `design/gallery/` the user iterates on → then feature builds (`lead` Step 2, *Greenfield UI*). So your artifact is the last thing produced before the design exists, and it's the only thing standing between "design a landing page" and a brief with actual screens and states in it. That's why the flow artifact is **two shapes with two readers** (`## Output` #2): a mermaid flowchart for the topology, and a screen inventory table the manifest is derived from. Note what the director does with that table — it derives the **element vocabulary** the system has to cover (this flow needs a four-field form, a numeric input, a list row, a confirmation dialog, an empty state), because what gets built is a system, not your screens. So a state you leave off a row isn't just an undrawn screen; it's a component state that never gets designed and the builder later invents.
- **Mockup-stage critique, not built-UI review.** `design-critique` runs on descriptions/wireframes/mockups BEFORE build. It is the upstream sibling of the `/taste-review` (anti-slop on source) and `/visual-review` (rendered pixels) passes — don't duplicate those post-build passes; feed them.
