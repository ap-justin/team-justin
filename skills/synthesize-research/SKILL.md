...
<!-- vendored from the knowledge-work `product-management` plugin (v1.2.0, sha 15898ec).
     copied into the repo so the `product-manager` seat keeps this command without the plugin's 16
     auth-gated MCP servers loaded. upstream ships it namespaced as `product-management:synthesize-research`;
     this is the plain vendored copy, invoked as `/synthesize-research`. re-sync: diff vs the plugin cache after an update. -->

# Synthesize Research

> Connector-gated steps below are optional; in this team they map to `gh`/`TRACKER.md` + the seat's wired Linear/Sentry read tools.
