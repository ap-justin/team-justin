...
<!-- vendored from `leonxlnx/taste-skill` (main, sha 7c397f2) → `skills/design-taste-frontend`. tasteskill anti-slop frontend pack backing the `design-director` seat + `/taste-review` skill. NOTE: upstream is leonxlnx/taste-skill (MIT), NOT mattpocock/skills. Kept verbatim; re-sync: re-download from https://raw.githubusercontent.com/leonxlnx/taste-skill/main/skills/taste-skill/. -->
...
### 3.A Stack
<!-- team divergence: this section is READ AS EXAMPLES, NOT AS THE TEAM'S STACK. re-check on every refresh or the sync silently reverts the note.
     upstream is a React/Next pack; the team's default framework target is Svelte 5 / SvelteKit, and its two readers can't act on this section anyway —
     `design-director` writes plans and never code, and `/taste-review` is report-only. brownfield always defers to the repo's own conventions (team principle: minimal diff).
     what survives translation is the *reasoning*: dvh over vh, grid over flex-percentage math, self-hosted fonts, isolate interactivity to leaves,
     never track continuous pointer/scroll values in re-render state, verify a dependency exists before importing it. transliterate those into whatever stack is actually present.
     what does NOT bind: the named packages (React/Next/RSC, Tailwind v4, Motion, `next/font`, Zustand/Jotai) and the Tailwind class syntax used throughout §4. -->
...
### 3.C Icons
<!-- team divergence: upstream discourages lucide and ranks phosphor first. the team defaults to lucide; see SOURCES.md → Vendored resources. -->
* **Team default:** `lucide` (`lucide-react` / `lucide-svelte` / the framework's binding). `design-director` names the set in its plan along with the grid, stroke weight, and allowed sizes; the builder installs what the plan names and nothing else.
* **Alternatives (priority order), when the brief or the brand calls for a different alphabet:** `@phosphor-icons/react`, `hugeicons-react`, `@radix-ui/react-icons`, `@tabler/icons-react`. A project that already depends on one of these keeps it — brownfield defers, as everywhere else.
* **NEVER hand-roll SVG icons.** If a glyph is missing, install a second library or compose from primitives - do not draw icon paths from scratch.
* **One family per project.** Do not mix two icon families in the same component tree.
...
* **NO generic avatars.** No SVG "egg", no icon-library user glyph standing in for a face → use believable photo placeholders or specific styling.
...
* **NO hand-rolled SVG icons.** Use the set the design plan names — Lucide by default, or one of the alternatives in Section 3.C.
...
<!-- team divergence: THE BLOCK LIBRARY DOES NOT EXIST IN THIS VENDORED COPY. re-check on every refresh.
     upstream never shipped the blocks either — the section is a forward-looking contract, and the path below (`skills/taste-skill/blocks/`)
     is upstream's layout, not this repo's (`skills/design-taste-frontend/` holds SKILL.md + LICENSE, nothing else).
     so: do not attempt to open a block file, and do not report a missing one as a defect.
     §10's Reference Vocabulary is the part that works — it names the patterns, which is what `design-director` needs to specify a layout treatment.
     if a refresh ever brings real blocks, drop this note and re-point the path. -->
...
- [ ] **Icons** from the one set the design plan names (Lucide by default — Section 3.C), one family only, no hand-rolled SVG paths?
