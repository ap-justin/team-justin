...
7. **Countable claims in `ROSTER.md` prose** — every number a skill row asserts about its own files is re-derived, not trusted: corpus entry counts (`grep -c "^## " skills/<name>/reference/*.md`), `Detect:` tier breakdowns, group counts, verified-URL counts. These drift silently in the opposite direction from the version header — a release bumps `VERSION` and the prose keeps the old figure, so the row reads authoritative and is wrong. Live example: `ui-principles` said **51 entries · 48 STATIC** through three releases that added eight. Re-count rather than reading the sentence.
