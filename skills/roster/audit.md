...
6. **Shared-block integrity** — the assertions above are all *registration*; this one is the **prompt body**, where drift actually lands (a seat can be perfectly registered and still have lost a rule). Against `shared-blocks.md`:
   ...
   - each block's **invariant clauses** are present verbatim, especially the six marked ⚠ (the reading-around-to-*find*-code trigger, `to hundreds of K tokens`, `Never write the whole test file up front`, `A **bug fix has no exemption**`, and C.1's cap-plus-drop-notice and `Never capped, always inline`) — a shortened bullet is the drift, not a style choice
...
7. **Countable claims in `ROSTER.md` prose** — every number a skill row asserts about its own files is re-derived, not trusted: corpus entry counts (`grep -c "^## " skills/<name>/reference/*.md`), `Detect:` tier breakdowns, group counts, verified-URL counts. These drift silently in the opposite direction from the version header — a release bumps `VERSION` and the prose keeps the old figure, so the row reads authoritative and is wrong. This has happened: the since-removed `ui-principles` row said **51 entries · 48 STATIC** through three releases that added eight, and was still wrong at deletion. Re-count rather than reading the sentence.
