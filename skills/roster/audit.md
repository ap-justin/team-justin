...
6. **Shared-block integrity** — the assertions above are all *registration*; this one is the **prompt body**, where drift actually lands (a seat can be perfectly registered and still have lost a rule). Against `shared-blocks.md`:
   ...
   - each block's **invariant clauses** are present verbatim, especially the four marked ⚠ (the reading-around-to-*find*-code trigger, `to hundreds of K tokens`, `Never write the whole test file up front`, and `A **bug fix has no exemption**`) — a shortened bullet is the drift, not a style choice
