...
<!-- vendored from the knowledge-work `product-management` plugin (v1.2.0, sha 15898ec).
     ...
     auth-gated MCP servers loaded. upstream ships it namespaced as `product-management:metrics-review`;
     this is the plain vendored copy, invoked as `/metrics-review`. re-sync: diff vs the plugin cache after an update. -->
