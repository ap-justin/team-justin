...
<!-- vendored verbatim from `cloudflare/skills` main:skills/wrangler (Apache-2.0; LICENSE kept).
     the Wrangler CLI skill (deploy/dev/manage Workers, KV, R2, D1, Queues, Workflows, Secrets Store).
     backs the `cloudflare-builder` seat's deploy/config surface, alongside the `cloudflare` platform skill,
     the connected Cloudflare MCP + Context7. not installed via a plugin here, so vendored like react-router.
     re-sync: re-download skills/wrangler/SKILL.md from the cloudflare/skills repo. symlinked at ~/.claude/skills/wrangler. -->
