...
- **`@testing-library/react` leaks renders here too** (1 button, then 2 across two tests — same cause as under jsdom). `render` from `vitest-browser-react` cleans up between tests without a hook: 0 elements left over.
