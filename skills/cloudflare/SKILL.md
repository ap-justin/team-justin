...
<!-- vendored verbatim from `cloudflare/skills` main:skills/cloudflare (Apache-2.0; LICENSE kept).
     the comprehensive first-party Cloudflare platform skill (Workers, Pages, KV/D1/R2, Durable Objects, Queues,
     Workers AI, Vectorize, WAF, Tunnel, …) — progressive disclosure: this SKILL.md indexes references/*, loaded
     on demand. backs the `cloudflare-builder` seat alongside the connected Cloudflare MCP + Context7.
     not installed via a plugin here, so vendored like react-router. kept verbatim.
     re-sync: re-download skills/cloudflare/ (SKILL.md + references/) from the cloudflare/skills repo.
     symlinked at ~/.claude/skills/cloudflare. -->
