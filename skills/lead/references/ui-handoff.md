...
The vocabulary itself is **shadcn's semantic set** as plain custom properties — the naming only, no shadcn or Tailwind dependency. `design-director` owns the contract; the builders read the names off the real token file (`skills/roster/shared-blocks.md` → Block E is the canonical seat-facing text). A handoff points at that file and never restates its values.
