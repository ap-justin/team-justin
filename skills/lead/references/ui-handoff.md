...
What *is* machine-checkable is the source: a literal hex, an off-scale `p-[13px]`, an ad-hoc duration are all greppable — and that is the **repo's own gate**, written by the builder in Phase 0 and run at every commit, not a review pass anyone dispatches (`ui-practice.md` → *The conformance gate is a deliverable*). A gate in the suite runs in no context window and can't be skipped because the batch was busy. Judgment stays with the user; enforcement stays static and local.
