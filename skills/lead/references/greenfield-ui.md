...
9. **Then feature work dispatches** — domain screens composed from the vocabulary against `ux-designer`'s inventory. Composition is the builder's engineering work; what it may not do is reach outside the token file for a value (*Conformance is prevented, not detected*, `ui-handoff.md`).
