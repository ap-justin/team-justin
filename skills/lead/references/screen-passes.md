...
## The three passes have seats, and you dispatch them

`taste-reviewer`, `visual-reviewer` and `accessibility-reviewer` are review-only agents whose entire body is the corresponding skill file — same rules, same output, same boundaries, read from the one copy. **Two callers, one body:** the user still types `/taste-review` · `/visual-review` · `/accessibility-review` and gets the pass inline; you spawn the seat and get it in an isolated context, in parallel with your other Step 4 gates.

What that reversal does **not** touch is what a pass may conclude. `visual-reviewer` returns **breakage and causes, never a verdict on intent** — whether the build looks like the design stays the user's glance, and it's prevented upstream by a closed token file (*Conformance is prevented, not detected*, Step 3). Dispatching the pass buys you coverage and rule violations; it does not buy you the design gate, and it never replaces the user's look.
...
- **`taste-reviewer` first on a token-shaped defect** — a literal hex or an off-scale value is greppable in seconds, and finding it in a browser costs a hundred times more.
...
These are cheap to run after the fact and each carries its own **taste** — which is exactly why they stayed the user's when the three conformance passes didn't: a copy rewrite or a motion roadmap is a preference, and dispatching one as a gate means the team quietly editing the user's voice.
