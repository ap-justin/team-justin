...
**`/design` is the surface.** The lead runs it and hands back a link: the artboards are drafted here from the repo, the user reshapes them in the editor and **Save** republishes. Any stack, no setup, and the design gate — **the user's eye on a render** — lands in the same turn instead of out of band (*The canvas*, below).
...
3. **The look gets settled — on a canvas, in front of the user.** The only human-in-the-loop stop in the build, and **the design gate**: the look is settled at the cheapest point it will ever be settled. Run **`/design`** in this session, briefed with the inventory and the corpus, putting up 2–4 genuinely different direction artboards for the user to pick one they can see. A user who would rather work in their own claude.ai/design session gets the two artifacts handed over in one line instead; say plainly you're waiting. Either way put everything non-styling in flight meanwhile (scaffold, toolchain, data layer) — a canvas turn is short, a user's own session is not.
