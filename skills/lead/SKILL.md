...
- **`${CLAUDE_PLUGIN_ROOT}/TRACKER.md`** — the file-based brief/plan/todos/issues/notes convention `/kru:brief` + `planner` write against; the store lives at user level (`~/.claude/kru/management/<project-slug>/`), never in the working repo — not even a pointer to it in the repo's `CLAUDE.md`.
