...
**The scan — run it over the brief's words before you send it.** The seven items are how you compose one; these four are text-level checks on what you actually wrote, and what they catch is what recurs. Scans 1, 2 (its verbatim half) and 4 have an enforcer: `hooks/check-handoff.sh` refuses a team-seat dispatch whose prompt carries a `file:line` or `line N` coordinate, a verbatim run from an always-loaded text, or no `inbox.md`, and hands the reason back — fix the brief and dispatch again.
