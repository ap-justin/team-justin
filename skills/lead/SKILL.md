...
**Route each file to a seat: `${CLAUDE_PLUGIN_ROOT}/references/routing.md`** — the detected-stack table and the contested-lane tie-breaks. Where the repo's block already names the seat, that is the answer and this file is not needed; open it for a stack the block doesn't cover, or a repo with no block at all.
